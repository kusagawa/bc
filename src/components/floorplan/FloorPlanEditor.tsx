...
import { Plus, Save, ZoomIn, ZoomOut, RotateCcw, Move, Square, Trash2, Grid3X3 } from 'lucide-react';
...
export default function FloorPlanEditor() {
  const { floorPlans, updateFloorPlan, selectedStoreId, gondolas } = useAppContext();
  ...
  const [newArea, setNewArea] = useState<{ category: string }>({ category: '飲料' });
  const svgRef = useRef<SVGSVGElement>(null);

  const selectedArea = plan?.areas.find(a => a.id === selectedAreaId) || null;

  const handleAreaMouseDown = (e: React.MouseEvent, area: FloorArea) => {
  ...
  const handleResizeMouseDown = (e: React.MouseEvent, area: FloorArea) => {
    e.stopPropagation();
    setResizeState({
      areaId: area.id,
      startX: e.clientX,
  ...
  return (
    <div className="flex gap-3 h-[calc(100vh-120px)]">
          ...
          {plan.areas.map(area => (
            <button
              ...
              className={`w-full text-left p-2 rounded-md text-xs transition-colors border-none cursor-pointer flex items-center gap-2 ${
                selectedAreaId === area.id ? 'bg-blue-50 text-blue-800' : 'hover:bg-gray-50 text-gray-700 bg-white'
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-gray-400">{area.width}×{area.height}</div>
              </div>
            </button>
          ))}
        </div>
          ...
          <div className="border-t border-gray-200 p-3 space-y-2">
            ...
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded px-2 py-1 text-xs mt-0.5 focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
            <div className="grid grid-cols-2 gap-1.5 text-[10px]">
              <div>
                <span className="text-gray-500">X: {selectedArea.x}</span>
              </div>
              <div>
                <span className="text-gray-500">Y: {selectedArea.y}</span>
              </div>
              <div>
                <span className="text-gray-500">幅: {selectedArea.width}</span>
              </div>
              <div>
                <span className="text-gray-500">高さ: {selectedArea.height}</span>
              </div>
            </div>
            <div className="text-[10px] text-gray-500">
              配置ゴンドラ: {selectedArea.gondolaIds.length}本
            </div>
            <button
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`p-1.5 rounded text-xs border-none cursor-pointer ${
                tool === 'select' ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-200 bg-transparent'
              }`}
              title="選択"
            >
              <Move size={14} />
            ...
            <div className="relative">
              <button
                ...
                className={`p-1.5 rounded text-xs border-none cursor-pointer flex items-center gap-1 ${
                  tool === 'add' ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-200 bg-transparent'
                }`}
                title="エリア追加"
              >
                <Plus size={14} />
            ...
            {tool === 'add' && (
              <select
                ...
                className="text-xs border border-gray-300 rounded px-1.5 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 ml-1"
              >
            ...
            <button onClick={() => setZoom(0.8)} className="p-1.5 text-gray-500 hover:bg-gray-200 rounded bg-transparent border-none cursor-pointer">
              <RotateCcw size={14} />
            </button>
            <div className="w-px h-5 bg-gray-300 mx-1" />
            <button className="flex items-center gap-1 px-2 py-1 text-xs bg-blue-600 text-white rounded-md hover:bg-blue-700 border-none cursor-pointer">
              <Save size={12} />
              保存
            </button>
          </div>
      ...
      {/* Right Panel: Gondola assignment */}
      <div className="w-52 bg-white rounded-lg border border-gray-200 flex flex-col overflow-hidden shrink-0">
        <div className="px-3 py-2 border-b border-gray-200 bg-gray-50">
          <h2 className="text-xs font-semibold text-gray-700">ゴンドラ配置</h2>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {storeGondolas.map(g => (
            <div key={g.id} className="p-2 rounded-md bg-gray-50 text-xs">
              <div className="font-medium text-gray-700 truncate">{g.name}</div>
              <div className="text-gray-400 mt-0.5">{g.category} / {g.status}</div>
            </div>
          ))}
